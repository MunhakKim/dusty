...
app.post('/message', function (req, res) {
    ...
    switch (msg) {
        case '서울':
            send = {
                'message': {
                    'text': '중구  한강대로  종로구  청계천로  종로  용산구  광진구  성동구  강변북로  중랑구  동대문구  홍릉로  성북구  정릉로  도봉구  은평구  서대문구  마포구  신촌로  강서구  공항대로  구로구  영등포구  영등포로  동작구  동작대로 중앙차로  관악구  강남구  서초구  도산대로  강남대로  송파구  강동구  천호대로  금천구  강북구  양천구  노원구  화랑로'
                }
            }
            res.json(send);
            break;
        ...
        case '인천':
            send = {
                'message': {
                    'text': '청라  송도  신흥  송림  구월동  숭의  석바위  부평역  부평  연희  검단  계산  고잔  석남  송해  동춘  운서  송현  논현  원당  석모리  덕적도  백령도'
                }
            }
            res.json(send);
            break;
        ...
        case '대구':
            send = {
                'message': {
                    'text': '좌동  진천동  이곡동  시지동  수창동  지산동  서호동  이현동  평리동  대명동  노원동  신암동  태전동  만촌동  호림동  현풍면'
                }
            }
            res.json(send);
            break;
        ...
        case '광주':
            send = {
                'message': {
                    'text': '서석동  농성동  치평동  두암동  운암동  건국동  송정1동  오선동  주월동  경안동'
                }
            }
            res.json(send);
            break;
        ...
        case '대전':
            send = {
                'message': {
                    'text': '읍내동  문평동  문창동  구성동  노은동  대흥동1  성남동1  정림동  둔산동  월평동'
                }
            }
            res.json(send);
            break;
        ...
        case '울산':
            send = {
                'message': {
                    'text': '화산리  상남리  농소동  삼남면  대송동  성남동  부곡동(울산)  여천동(울산)  야음동  삼산동  도로변  신정동  덕신리  무거동  효문동  약사동'
                }
            }
            res.json(send);
            break;
        ...
        case '경북':
            send = {
                'message': {
                    'text': '칠곡군  장흥동  장량동  대도동  대송면  3공단  성건동  문당동  남문동  공단동  원평동  형곡동  4공단  휴천동  중방동  지품면  화북면  안계면  태하리  상주시'
                }
            }
            res.json(send);
            break;
        ...
        case '경남':
            send = {
                'message': {
                    'text': '반송로  사파동  경화동  하동읍  동상동  삼방동  장유동  저구리  아주동  사천읍  대산면  북부동  웅상읍  남상면  전포동  회원동  봉암동  상봉동  대안동  상대동  명서동  웅남동  가음정동  용지동  내일동  무전동'
                }
            }
            res.json(send);
            break;
        ...
        case '충북':
            send = {
                'message': {
                    'text': '옥천읍  영동읍  증평읍  진천읍  송정동(봉명동)  사천동  문화동  용암동  복대동  호암동  칠금동  장락동  오창읍  매포읍  청천면  금왕'
                }
            }
            res.json(send);
            break;
        ...
        case '충남':
            send = {
                'message': {
                    'text': '성거읍  배방읍  송산면  금산읍  서천읍  서면  당진시청사  도고면  둔포면  인주면  대산리  청양읍  엄사면  성황동  백석동  성성동  사곡면  독곶리  동문동  모종동  파도리  공주  부여읍  논산  이원면  예산군  대천2동  홍성읍  태안읍'
                }
            }
            res.json(send);
            break;
        ...
        case '전북':
            send = {
                'message': {
                    'text': '중앙동(전주)  진안읍  고산면  요촌동  부안읍  삼천동  팔복동  금암동  신풍동(군산)  소룡동  개정동  남중동  팔봉동  모현동  연지동  죽항동  고창읍  운암면  새만금'
                }
            }
            res.json(send);
            break;
        ...
        case '전남':
            send = {
                'message': {
                    'text': '용당동  부흥동  서강동  월내동  문수동  여천동(여수)  덕충동  장천동  연향동  순천만  호두리  중동  태인동  진상면  광양읍  나불리  송단리  해남읍  담양읍  장성읍  빛가람동'
                }
            }
            res.json(send);
            break;
        ...
        default:
            request(makeUrl(msg), function (error, response, body) {
                ...
                if (totalCnt == 0) {
                    send = {
                        'message': {
                            'text': '전국 키워드 or 측정소를 입력하세요. 전국 키워드를 입력할 경우 측정소 목록을 볼 수 있습니다. 측정소를 입력할 경우 미세먼지의 상태를 알수 있답니다. ex) 전국: 서울,인천,경기,경북 등, 측정소: 종로구, 성건동 등'
...
http.createServer(app).listen(5974, function () {
    console.log('서버 실행 중...\n')
});
